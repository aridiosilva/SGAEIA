...
class NatsBusAdapter(BaseIntegrationAdapter):
    """SGAEIA IP-BUS adapter for NATS/JetStream-style control/event messaging."""

    fail_mode = "buffer-and-retry"

    def __init__(self, transport: JsonTransport, *, implementation_version: str = "2.14.5", max_buffer: int = 1000):
        self.transport = transport
        self.max_buffer = max_buffer
        self.buffer: list[dict[str, Any]] = []
        self._revoked: set[str] = set()
        self.metadata = AdapterMetadata(
            "ADP-NATS-BUS", "IP-BUS", "NATS/JetStream", implementation_version,
